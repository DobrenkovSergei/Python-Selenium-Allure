...
def test_right_way(browser):
    ...
    WebDriverWait(browser, timeout=10, poll_frequency=1).until(
        EC.url_to_be("https://test.qa.studio/checkout/"))   #https://test.qa.studio/?page_id=10
    ...
    WebDriverWait(browser, timeout=5, poll_frequency=1).until(
        EC.url_changes("https://test.qa.studio/checkout/order-received/12689/?key=wc_order_GIuPMN6Yjmi5D")) 
